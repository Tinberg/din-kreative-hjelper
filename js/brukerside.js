...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchUserProfile() {
        ...
        .then(data => {
            ...
            // If location data is present, display it
            if (data.location) {
                const coords = parseCoordinates(data.location);
                if (coords) {
                    displayLocation(data.location, data.location); // Display using the location string
                    initMap(coords.latitude, coords.longitude); // Initialize map with these coordinates
                }
            }
    ...
    function updateLocation(formattedAddress) {
        ...
        // Add code to update the location on the server
        // ...
