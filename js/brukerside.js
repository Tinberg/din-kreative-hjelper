...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let userSelectedAddress = localStorage.getItem('user_selected_address'); // Retrieve user-selected address from local storage
    ...
    function fetchUserProfile() {
        ...
        .then(data => {
            ...
            if (data.location) {
                ...
                if (coords) {
                    if (userSelectedAddress) {
                        displayLocation(userSelectedAddress, data.location); // Use the stored address
                    } else {
                        reverseGeocodeAndDisplay(coords.latitude, coords.longitude, data.location);
                    }
                }
