...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchUserProfile() {
        ...
        .then(data => {
            ...
            if (data.location) {
                const coords = parseCoordinates(data.location);
                if (coords) {
                    const storedAddress = localStorage.getItem('user_selected_address');
                    if (storedAddress) {
                        displayLocation(storedAddress, data.location);
                    } else {
                        reverseGeocodeAndDisplay(coords.latitude, coords.longitude, data.location);
                    }
                }
    ...
    function updateLocation(formattedAddress) {
        ...
        // Lagre både det formaterte adressenavnet og koordinatene
        localStorage.setItem('user_selected_address', formattedAddress);
        localStorage.setItem('user_selected_coordinates', tempCoordinates);
